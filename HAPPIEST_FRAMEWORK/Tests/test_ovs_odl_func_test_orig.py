...
class test_OVS_ODL_FUNC_Test(unittest.TestCase):
    ...
    def test_4_vlan_based_flows_test(self):
        ...
        self.ovs_obj.manage_flows(manage_type="delete", br_name=config.OVS_BRIDGE, flow_inputs=None,
        ...
        self.ovs_obj.manage_flows(manage_type="add", br_name=config.OVS_BRIDGE, flow_inputs=flow_input,
                                  controller=config.CONTROLLER_TYPE)
        self.tg = Scapy(device_obj=obj1, node=node1)
        packet = self.tg.generate_packet_to_send(pkt_type='vlan')
        output = self.h1.send_pkts_n_capture(scapy_obj=self.tg, node=obj2, src_if=config.HOST1_PORTS[0],
                                             dst_if=config.HOST2_PORTS[0], pkt=packet, count=config.pkt_count,
...
#        self.ovs_obj.manage_flows(manage_type="delete", br_name=config.OVS_BRIDGE, flow_inputs=flow_input,
#                                  controller=config.CONTROLLER_TYPE)
    ...
    def test_5_vlan_based_flows_test(self):
        ...
        self.ovs_obj.manage_flows(manage_type="delete", br_name=config.OVS_BRIDGE, flow_inputs=flow_input,controller=config.CONTROLLER_TYPE)
    ...
    def test_6_port_based_flows_test(self):
        ...
        if self.mini:
            ...
            dest = config.HOST2_IPs[0]
        else:
            obj = self.h1
            source = None
            dest = config.HOST2_IPs[0]

        status = self.ovs_obj.manage_flows(manage_type="delete", br_name=config.OVS_BRIDGE, flow_inputs=None,
                                  controller=config.CONTROLLER_TYPE)
        ...
        status = self.ovs_obj.manage_flows(manage_type="add", br_name=config.OVS_BRIDGE, flow_inputs=flow_input,
                                  controller=config.CONTROLLER_TYPE)
        if status is False: assert False
        ...
        status = self.ovs_obj.manage_flows(manage_type="delete", br_name=config.OVS_BRIDGE, flow_inputs=flow_input,
                                  controller=config.CONTROLLER_TYPE)
        if status is False: assert False
    ...
    def test_7_ACL_Rules_based_flows_test(self):
        ...
        if not self.mini:
            ...
            if self.h1.execute_command_host(cmd='ssh -o ConnectTimeout=10 %s@%s'
                                             %(config.HOST1_USER,config.HOST2_IPs[0].split('/')[0]),exp_out='yes/no'):
            ...
            if status is False: assert False
            if self.h1.execute_command_host(cmd='ssh -o ConnectTimeout=10 %s@%s'
                    % (config.HOST1_USER, config.HOST2_IPs[0].split('/')[0]), exp_out='Connection timed out'):
