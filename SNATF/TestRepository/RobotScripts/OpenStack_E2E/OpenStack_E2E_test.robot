...
Variables         ../../../Framework/NFV/PythonLib/OpenStack_E2E/test_config_crs.py
...
OPENSTACK - Life Cycle Management: Create Instance Has Single Network
    ...
    Set Environment     ${auth}
    ${status}    Create Stack Test    ${stack_name}  ${yaml_loc}  ${yaml_name}  ${mgmt_nw}  ${base_image}  ${flavor}  ${public_key}  ${key_loc}  ${username}  ${password}  ${sec_name}
    should be equal    ${status}    ${True}
...
OPENSTACK - Life Cycle Management: Create Instance Has Multi Networks
    ...
    Set Environment     ${auth}
    ${status}    Create Stack Multi Network Test    ${stack2_name}  ${yaml_loc}  ${yaml2_name}  ${mgmt_nw}  ${network_name}  ${base_image}  ${flavor}  ${public_key}  ${key_loc}  ${username}  ${password}  ${sec_name}
    should be equal    ${status}    ${True}
...
OPENSTACK - Life Cycle Management: Upgrade Instance Has Multi Networks With Image
    ...
    Set Environment     ${auth}
    ${status}    Upgrade Stack Test    ${stack2_name}  ${yaml_loc}  ${ug_yaml_name}  ${base_image}
    should be equal    ${status}    ${True}
...
OPENSTACK - Life Cycle Management: Create Multiple Instances Having Multi Networks Through Single Yaml
    ...
    Set Environment     ${auth}
    ${status}    Create Stack Multi Network Multi Server Test   ${stack3_name}  ${yaml_loc}  ${yaml3_name}  ${mgmt_nw}  ${network_name}  ${base_image}  ${flavor}  ${public_key}  ${key_loc}  ${username}  ${password}  ${sec_name}
    should be equal    ${status}    ${True}
