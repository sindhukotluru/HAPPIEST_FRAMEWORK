...
Variables         ../Config/MEC_Config.py
Variables         ../Config/MEC_REST_INPUT_Config.py
Variables         ../Config/MEC_ControllerConfig.py
Library           Collections
Library           String
...
PreRequisite
    ...
    import Library    MEC.ovs    IP=${SWITCH['IP']}    username=${SWITCH['USER']}
    ...     password=${SWITCH['PASSWORD']}    WITH NAME    OVSOBJ
    ${ovs_obj}    Get Library Instance    OVSOBJ
    set suite variable    ${ovs_obj}
    Config MEC Infra
    import Library    RestAPILib.rest_utils.REST     content_type= ${CONTENT_TYPE}    WITH NAME    RESTOBJ
...
GetAndValidateStats
    ...
    ${cmd}=   Set Variable    /home/test/intdpc02/MEC/BkEnd/InstallScripts/ovs_tele.py --in-port 1 --out-port 2 > /var/ovs_tele.log &
    Run Keyword If    ${status}==${True}    OVSOBJ.Ovs_Execute_Command    cmd=${cmd}
    ...
    import Library    GenericLib.hosts    IP=${HOST1['IP']}    username=${HOST1['USER']}
    ...     password=${HOST1['PASSWORD']}    WITH NAME    SD
    ${sd_obj}    Get Library Instance    SD
    import Library    TraficLib.Traffic_Generator.Scapy    ${sd_obj}    None    WITH NAME    TG
...
CleanConfigInfra
    OVSOBJ.Ovs_Execute_Command    cmd=killall node
    OVSOBJ.Ovs_Execute_Command    cmd=kill $(ps aux | grep 'main.py' | awk '{print $2}')
    OVSOBJ.Ovs_Execute_Command    cmd=ps -ef | grep "main.py"
    ${pattern}    Set Variable   --redis-address
    Should Not Match Regexp    ${ovs_obj.resp}    ${pattern}
    OVSOBJ.Ovs_Execute_Command    cmd=ps -ef | grep node
    Should Not Match Regexp    ${ovs_obj.resp}    index
    
Config MEC Infra
    CleanConfigInfra
    #${cmd}=   Set Variable    cd /home/test/MEC/MEC/BkEnd/InstallScripts/;./config_infra_and_apps.py;cd -  
    ${cmd}=   Set Variable    cd /home/test/intdpc02/MEC/BkEnd/InstallScripts/;./config_infra_and_apps.py;cd -  
    OVSOBJ.Ovs_Execute_Command    cmd=${cmd}
    sleep    3s 
    OVSOBJ.Ovs_Execute_Command    cmd=ps -ef | grep "main.py"
    ${pattern}    Set Variable   --redis-address
    Should Match Regexp    ${ovs_obj.resp}    ${pattern}
    ${cmd}=   Set Variable   node /home/test/intdpc02/MEC/BkEnd/InstallScripts/index.js > /var/log/index.log &
    OVSOBJ.Ovs_Execute_Command    cmd=${cmd}
    OVSOBJ.Ovs_Execute_Command    cmd=ps -ef | grep node
    Should Match Regexp    ${ovs_obj.resp}    index
    Log    MEC Infra Started Successfully
