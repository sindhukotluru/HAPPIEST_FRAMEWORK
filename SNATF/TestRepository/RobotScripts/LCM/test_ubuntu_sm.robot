...
Library           SSHLibrary
Library		      random
Library           ../../../Framework/NFV/PythonLib/LCM/util_findhost.py
Library           ../../../Framework/NFV/PythonLib/LCM/orch_lib_sm.py
Variables         ../../../Framework/NFV/PythonLib/LCM/config_ubuntu_sm.py
...
OPENSTACK - Validate Vnf Orchestration: Upgrade Multi Network VM Image
    [Documentation]    Test Upgrade Multi Network VM Image
    Set Environment     ${auth}
    ${status}    Upgrade Stack Test    ${stack2_name}  ${yaml_loc}  ${ug_yaml_name}  ${upgrade_image}
    should be equal    ${status}    ${True}

OPENSTACK - Validate Vnf Orchestration: Downgrade Multi Network VM Image
    [Documentation]    Test Downgrade Multi Network VM Image
    Set Environment     ${auth}
    ${status}    Downgrade Stack Test    ${stack2_name}  ${yaml_loc}  ${dg_yaml_name}  ${base_image}
    should be equal    ${status}    ${True}
...
OPENSTACK - Validate Vnf Orchestration: Create Multi Network Multi Server VMs in single Yaml
    ...
    Set Environment     ${auth}
    ${status}    Create Stack Multi Network Multi Server Test   ${stack3_name}  ${yaml_loc}  ${yaml3_name}  ${public_nw}  ${network_name}  ${base_image1}  ${flavor}  ${public_key}  ${key_loc}  ${username}  ${password}
    should be equal    ${status}    ${True}

OPENSTACK - Validate Vnf Orchestration: ICMP Traffic Test on Multi Network Multi Server VMs in single Yaml
    [Documentation]    Test ICMP Traffic on Multi Network Multi Server VMs in single Yaml
    Set Environment     ${auth}
    ${status}    Multi Network Multi Server Icmp Test    ${stack3_name}  ${username}  ${key_loc}  ${public_key}  ${password}
    should be equal    ${status}    ${True}

OPENSTACK - Validate Vnf Orchestration: HTTP Traffic Test on Multi Network Multi Server VMs in single Yaml
    [Documentation]    Test HTTP Traffic on Multi Network Multi Server VMs in single Yaml
    Set Environment     ${auth}
    ${status}    Multi Network Multi Server Http Traffic Test    ${stack3_name}  ${username}  ${key_loc}  ${public_key}  ${password}
    should be equal    ${status}    ${True}
