...
Library           ../../../Framework/NFV/PythonLib/LCM/orch_lib.py
Variables         ../../../Framework/NFV/PythonLib/LCM/config_ubuntu.py
...
OPENSTACK - Validate Vnf Orchestration: Create Multi Network Multi Server VMs in single Yaml
    ...
    Set Environment     ${auth}
    ${status}    Create Stack Multi Network Multi Server Test   ${stack3_name}  ${yaml_loc}  ${yaml3_name}  ${public_nw}  ${network_name}  ${base_image2}  ${flavor}  ${public_key}  ${key_loc}  ${username}  ${password}
    should be equal    ${status}    ${True}
