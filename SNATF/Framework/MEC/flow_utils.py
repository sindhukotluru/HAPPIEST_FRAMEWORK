...
import MEC_Config, MEC_ControllerConfig
from MEC.flows import FLOW
...
class FlowManagement(object):
    ...
    def add_flows(self, flow_attrs):
        ...
        url = r'http://{0}:{1}{2}{3}'.format(ControllerConfig.CONTROLLER_INFO["IP"],
                                         ...
                                         ControllerConfig.REST_CON, ControllerConfig.CONFIG_FLOW_API)
    ...
    def clear_flows(self,flow_attrs=None):
        ...
        if flow_attrs is not None:
            ...
            flow_api = re.search(r'(/open.*openflow:\d+).*',ControllerConfig.CONFIG_FLOW_API).group(1)
            url = r'http://{0}:{1}{2}{3}'.format(ControllerConfig.CONTROLLER_INFO["IP"],
                                                 ControllerConfig.CONTROLLER_RESTCONF_INFO['RESTCONFPORT'],
                                                 ControllerConfig.REST_CON,flow_api)
