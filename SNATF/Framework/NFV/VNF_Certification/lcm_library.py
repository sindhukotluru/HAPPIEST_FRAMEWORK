...
def validate_ip_vnf(stack_name):
    ...
    c1 = SSHConnection(IP=ip_float, username="root", ssh_key="/home/test/automation/lcm/key234demo.pem")
    try:
	c1.connect()
    except:
...
def vnf_upgrade_instance(stack_name):
    if not upgrade_stack(stack_name, 'vnf_update.yml'):
	return False
    time.sleep(60)
    stack_create_status = parse_json('stackShow', stack_name, "stack_status_reason")
    return True if stack_create_status == "Stack UPDATE completed successfully" else False

def vnf_downgrade_instance(stack_name):
    if not downgrade_stack(stack_name, 'vnf.yml'):
        return False
    time.sleep(60)
    stack_create_status = parse_json('stackShow', stack_name, "stack_status_reason")
    return True if stack_create_status == "Stack UPDATE completed successfully" else False
