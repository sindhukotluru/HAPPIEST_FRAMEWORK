...
##########Sindhu################
def list_stack_and_delete_stack():
    cmd = "openstack stack list | awk 'NR!=2 {print $4}' "
    print "command=" ,cmd
    output = exec_sys_command(cmd).split()
    print output
    for stack in output:
        delete_stack(stack)
    time.sleep(15)
    check = exec_sys_command(cmd).split()
    if not check:
        return True
################################
...
def test_http_traffic(login_ip1, login_ip3, usrname, key, password):
    ...
    if c1.connect() and c3.connect():
        ...
        c1.execute_command("ifconfig ens3")
        ...
        cmd1 = 'sudo tcpdump -nqt -s 0 -A -i ens3 port 80 | grep "IP ' + c1_eth1_ip + '"'
        pat1 = c1_eth1_ip
        check1 = run_check_cmd(c1, cmd, pat)
