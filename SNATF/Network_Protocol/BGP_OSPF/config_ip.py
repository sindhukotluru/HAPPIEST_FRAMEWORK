...
class config_ip(object):
    ...
    def start_configure(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.config_ip_addr, args=[device[0],device[1],device[2]])
                ...
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.config_host_addr, args=[device[0],device[1],device[2]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
    ...
    def start_configure_loopback(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.config_lo_addr, args=[device[0],device[1]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def start_configure_ospf(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.config_ospf, args=[device[0],device[1],device[2],device[3],device[4]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                sleep(2)
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
    ...
    def start_ibgp(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.config_ibgp, args=[device[0],device[1],device[2],device[3]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def enable_sync(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.ibgp_sync, args=[device[0],device[1]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def ebgp_configure(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.enable_ebgp, args=[device[0],device[1],device[2],device[3],device[4],device[5],device[6]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def advertise_loopback(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.loopback_advertise, args=[device[0],device[1],device[2],device[3]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def establish_route(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.route_establish, args=[device[0],device[1],device[2]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def show_ospf_neighbor(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.show_ip_ospf, args=[device[0], device[1]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
    ...
    def show_bgp_summary(self, devices):
        ...
        for device in devices:
            if re.search('R.*',device[0]):
                threads_device["new_thread{0}".format(device[0])] = Thread(target=config_ip.show_bgp, args=[device[0], device[1]])
                threads_device["new_thread{0}".format(device[0])].start()
                threads_device["new_thread{0}".format(device[0])].name = "ConfigureRouter_" + str(device[0])
                list_threads.append(threads_device["new_thread{0}".format(device[0])])
        for _thread in list_threads:
