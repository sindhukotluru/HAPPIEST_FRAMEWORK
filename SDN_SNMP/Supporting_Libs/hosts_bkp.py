...
class hosts(SSHConnection):
    ...
    def send_pkts_n_capture(self,scapy_obj,node,src_if,dst_if,pkt,count,inter,filter):
        """
        This method is to send traffic using scapy and capture
        :return:
        """
        PID,cap_file = node.start_capture_on_host(iface=dst_if,filter=filter)
