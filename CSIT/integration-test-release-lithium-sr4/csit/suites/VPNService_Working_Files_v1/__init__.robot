...
*** Variables ***
${start1}         sudo mn \ --controller=remote,ip=${ODL_SYSTEM_IP_1} --custom custom.py --topo Switch1 --switch ovsk,protocols=OpenFlow13
${start2}         sudo mn \ --controller=remote,ip=${ODL_SYSTEM_IP_1} --custom custom.py --topo Switch2 --switch ovsk,protocols=OpenFlow13
...
Start Suite
    ...
    ${mininet1_conn_id_1}=    Open Connection    ${TOOLS_SYSTEM_IP}    prompt=${TOOLS_SYSTEM_PROMPT}    timeout=30s
    ...
    Write    ${start1}
    Read Until    mininet>
    ${mininet1_conn_id_2}=    Open Connection    ${TOOLS_SYSTEM_IP}    prompt=${TOOLS_SYSTEM_PROMPT}    timeout= 30s
    ...
    ${mininet2_conn_id_1}=    Open Connection    ${TOOLS_SYSTEM_2_IP}    prompt=${TOOLS_SYSTEM_PROMPT}    timeout=30s
    ...
    Write    ${start2}
    Read Until    mininet>
    ${mininet2_conn_id_2}=    Open Connection    ${TOOLS_SYSTEM_2_IP}    prompt=${TOOLS_SYSTEM_PROMPT}    timeout= 30s
