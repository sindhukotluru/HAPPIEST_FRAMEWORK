...
Variables         ../../variables/vpnservice/point_to_point.py    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}
Library           SSHLibrary
...
@{vpn_inst_values}    testVpn1    100:1    200:1    300:1
@{ietf_int_values}    s1-eth1    s1-gre1    s2-eth1    s2-gre1
@{vpn_int_values}    s1-eth1    testVpn1    10.0.0.1    00:00:00:00:00:01
...               s2-eth1    10.0.0.3    00:00:00:00:00:03
...
Verify flows
    ...
    @{flow_elements}    Create List    ${vpn_int_values[2]}    ${vpn_int_values[5]}
...
Verify groups
    ...
    [Tags]    verify in switch
    @{group_elements1}    Create List    ${vpn_int_values[3]}
    @{group_elements2}    Create List    ${vpn_int_values[6]}
...
Verify ping H1 H2
    [Documentation]    Verifies the ping between H1 and H2 which are in the same subnet. Ping should fail between hosts in same vpn.
    [Tags]    verify in switch
...
Verify ping H1 H3
    [Documentation]    Verifies the ping between the two hosts. Ping should succeed between hosts h1 and h3.
    [Tags]    verify in switch
    Switch Connection    ${mininet1_conn_id_1}
    Write    h1 ping 10.0.0.3 -c 5
    ${result}    Read Until    mininet>
    Log    ${result}
...
Verify FIB entries after delete
    ...
    [Tags]    Get
    @{fib_entries}=    Create List    ${vpn_int_values[2]}    ${vpn_int_values[5]}
...
#    Should Contain X Times    ${output}    table=20    2
    Should Contain X Times    ${output}    table=20    7
#    Should Contain X Times    ${output}    table=21    4
    Should Contain X Times    ${output}    table=21    3
