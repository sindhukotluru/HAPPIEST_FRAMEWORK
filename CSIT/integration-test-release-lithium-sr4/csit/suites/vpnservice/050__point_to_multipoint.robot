...
Variables         ../../variables/vpnservice/point_to_multipoint.py    ${TOOLS_SYSTEM_IP}    ${TOOLS_SYSTEM_2_IP}
Library           SSHLibrary
...
@{vpn_int_values}    s1-eth1    testVpn1    10.0.0.1    00:00:00:00:00:01    s2-eth1    10.0.0.3    00:00:00:00:00:03     s2-eth2    10.0.0.4    00:00:00:00:00:04
...
Verify FIB entries after create
    ...
    [Tags]    Get
    @{fib_entries1}=    Create List    ${vpn_int_values[2]}    ${vpn_int_values[5]}
    @{fib_entries2}=    Create List    ${vpn_int_values[2]}    ${vpn_int_values[8]}
    ...
    Wait Until Keyword Succeeds    3s    1s    Check For Elements At URI    ${OPERATIONAL_FIB_ENTRIES_VRF_TABLE}${vpn_inst_values[1]}    ${fib_entries2}

Verify flows
    [Documentation]    Verify flows in the switches
    [Tags]    verify in switch
    @{flow_elements}    Create List    ${vpn_int_values[2]}    ${vpn_int_values[5]}    ${vpn_int_values[8]}
...
Verify groups
    ...
    @{group_elements2}    Create List    ${vpn_int_values[9]}
...
Verify ping fail H2 H1
    [Documentation]    Verifies the ping between the two hosts. Ping should succeed between hosts in same vpn, (h1,h3) and (h2,h4).Ping between hosts in different vpns should fail (h1,h2) & (h3,h4). The commented out section is to be uncommented when ovs supports mpls over gre in datapath
    [Tags]    verify in switch
    ${ping_result}    Ping Between Hosts    ${mininet1_conn_id_1}    h2    h1    5
    Should Contain    ${ping_result}    100% packet loss

Verify ping pass H1 H4
    [Documentation]    Verifies the ping between the two hosts. Ping should succeed between hosts in same vpn, (h1,h3) and (h2,h4).Ping between hosts in different vpns should fail (h1,h2) & (h3,h4). The commented out section is to be uncommented when ovs supports mpls over gre in datapath
    [Tags]    verify in switch
    ${ping_result}    Ping Between Hosts    ${mininet1_conn_id_1}    h1    10.0.0.4    5
    Should Contain    ${ping_result}    0% packet loss
...
Verify FIB entries after delete
    ...
    [Tags]    Get
    @{fib_entries}=    Create List    ${vpn_int_values[2]}    ${vpn_int_values[5]}    ${vpn_int_values[8]}
...
#    Should Contain X Times    ${output}    table=21    4
    ...
    Should Contain X Times    ${output}    table=20    8
    Should Contain X Times    ${output}    table=21    5
