...
Create VPN Instance
    ...
    ${resp}    RequestsLibrary.Post Request    session    ${VPN_INST_API}    data=${body}
    Log    ${resp.content}
...
Verify VPN instance
    ...
    [Tags]    Get
    ${resp}    RequestsLibrary.Get Request    session    ${VPN_INST_API}vpn-instance/${vpn_inst_values[0]}/    headers=${ACCEPT_XML}
...
Create ietf vm interface
    ...
    ${resp}    RequestsLibrary.Post Request    session    ${IETF_INTF_API}    data=${body}
    Should Be Equal As Strings    ${resp.status_code}    204

Verify ietf vm interface
    [Documentation]    Verifies ietf interface created
    [Tags]    Get
    ${resp}    RequestsLibrary.Get Request    session    ${IETF_INTF_API}interface/${vm_int_values[0]}/    headers=${ACCEPT_XML}
    Should Be Equal As Strings    ${resp.status_code}    200
    Log    ${resp.content}
...
Create VPN interface
    ...
    ${resp}    RequestsLibrary.Post Request    session    ${VPN_INTF_API}    data=${body}
    Should Be Equal As Strings    ${resp.status_code}    204
...
Delete VPN Instance
    ...
    [Tags]    Delete
    ${resp}    RequestsLibrary.Delete Request    session    ${VPN_INST_API}vpn-instance/${vpn_inst_values[0]}/
    Should Be Equal As Strings    ${resp.status_code}    200
...
Delete vm ietf interface
    ...
    [Tags]    Delete
    ${resp}    RequestsLibrary.Delete Request    session    ${IETF_INTF_API}interface/${vm_int_values[0]}
    Should Be Equal As Strings    ${resp.status_code}    200
...
Ensure The Fib Entry Is Present
    ...
    ${resp}    RequestsLibrary.Get Request    session    ${OPERATIONAL_FIB_ENTRIES}    headers=${ACCEPT_XML}
    Should Be Equal As Strings    ${resp.status_code}    200
    Log    ${resp.content}
